...
export async function displayShopping(content, isLoggedIn) {
    ...
    function renderItemCard(item, categoryId) {
        ...
        const description = createElement("p", {
            style: { fontSize: "0.85rem", margin: "0.25rem 0", color: "#444" }
        }, ["Short product description goes here."]);

        const condition = categoryId === "second-hand"
            ? createElement("div", { style: { fontSize: "0.8rem", color: "#777" } }, ["Condition: Good"])
            : null;

        const pickup = item.pickup
            ? createElement("div", { style: { fontSize: "0.8rem", color: "#007700" } }, ["✔ Local Pickup Available"])
            : null;

        const bids = categoryId === "auctions"
            ? createElement("div", { style: { fontSize: "0.8rem", color: "#555" } }, [`${item.bids} bid${item.bids === 1 ? "" : "s"}`])
            : null;
