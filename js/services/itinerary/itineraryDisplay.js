...
function displayItinerary(isLoggedIn, divContainerNode) {
  ...
  function createItineraryListItem(itinerary) {
    ...
    const actions = [
      ...
      { text: 'Edit', handler: () => editItinerary(itinerary.itineraryid) },
  ...
  async function viewItinerary(id) {
    try {
      ...
      // Or: navigate(`/itinerary/${id}`); if you want SPA-style
    } catch (err) {
      console.error(err);
  ...
  async function editItinerary(id) {
    navigate(`/edit-itinerary?id=${id}`); // SPA-compatible navigation
  }
