...
export function renderClothingProduct(product, isLoggedIn, productType, productId, container) {
  let quantity = 1;
  ...
  const colorInfo = product.color ? `Color: ${product.color}` : "";

  const details = [
    createElement("h2", {}, [product.name]),
    createElement("p", {}, [product.description]),
    createElement("p", {}, [sizeInfo]),
    createElement("p", {}, [colorInfo]),
    createElement("p", {}, [`Price: ₹${product.price.toFixed(2)} / piece`]),
