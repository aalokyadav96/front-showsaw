...
// import  displayPlace  from "./displayPlace.js"; // assumes this exists

function renderPlaceDetails(isLoggedIn, content, place, isCreator, triggerBannerUpload) {
    content.innerHTML = "";
    ...
    const bannerImg = createElement("img", {
        src: place.banner ? `${SRC_URL}/placepic/${place.banner}` : DEFAULT_IMAGE,
        alt: place.name,
        loading: "lazy",
    });

    const bannerSection = createElement("section", { id: "place-banner", class: "placedetails" }, [bannerImg]);

    if (isCreator && typeof triggerBannerUpload === "function") {
        const uploadBannerBtn = Button("Upload New Image", "upload-banner-btn", {
            click: () => triggerBannerUpload(place.placeid),
        }, "buttonx");
        bannerSection.appendChild(uploadBannerBtn);
    }
